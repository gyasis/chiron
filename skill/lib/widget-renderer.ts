...
export function renderCodeRunner(spec: CodeRunnerWidget): string {
  ...
  return [
    ...
    ``,
    `    function runJs(code){`,
    `      // T162: run untrusted user code inside a sandboxed iframe.`,
    `      // sandbox="allow-scripts" (NO allow-same-origin) gives the frame an`,
    `      // opaque origin, so prompt-injected JS cannot read this page's`,
    `      // localStorage, cookies, or DOM. The code is delivered via postMessage`,
    `      // after load (never string-concatenated into source → no breakout), and`,
    `      // captured console output is posted back the same way.`,
    `      var frame = document.createElement('iframe');`,
    `      frame.setAttribute('sandbox', 'allow-scripts');`,
    `      frame.setAttribute('aria-hidden', 'true');`,
    `      frame.style.cssText = 'display:none';`,
    `      var token = 'cr_' + Date.now().toString(36) + '_' + Math.floor(Math.random()*1e9).toString(36);`,
    ...
    `        var d = ev.data;`,
    `        if (!d || d.__chironCr !== token) return;`,
    `        done = true;`,
    `        setOutput(String(d.out == null ? '' : d.out), d.ok ? '' : 'error');`,
    `        cleanup();`,
    `      }`,
    `      window.addEventListener('message', onMsg);`,
    `      var boot = "(function(){var buf=[];console.log=function(){var p=[];for(var i=0;i<arguments.length;i++){try{p.push(typeof arguments[i]==='string'?arguments[i]:JSON.stringify(arguments[i]));}catch(e){p.push(String(arguments[i]));}}buf.push(p.join(' '));};addEventListener('message',function(ev){var m=ev.data;if(!m||!m.__chironCode)return;var ok=true,out='';try{var r=eval(m.code);if(typeof r!=='undefined')buf.push(String(r));out=buf.join(String.fromCharCode(10));}catch(e){ok=false;out=(e&&e.message)?e.message:String(e);}parent.postMessage({__chironCr:m.token,ok:ok,out:out},'*');});})();";`,
    `      frame.srcdoc = '<!DOCTYPE html><meta charset=utf-8><scr'+'ipt>'+boot+'</scr'+'ipt>';`,
    `      frame.onload = function(){`,
    `        try { frame.contentWindow.postMessage({ __chironCode: true, code: code, token: token }, '*'); }`,
    `        catch(e) { if (!done) { done = true; setOutput('sandbox error', 'error'); cleanup(); } }`,
    `      };`,
    `      timer = setTimeout(function(){`,
    `        if (done) return; done = true;`,
    `        setOutput('Execution timed out (possible infinite loop).', 'error');`,
    `        cleanup();`,
    `      }, 5000);`,
    ...
    `        // T164: Subresource Integrity — browser refuses to execute the CDN`,
    `        // payload if its hash does not match (defends against CDN tampering).`,
    `        s.integrity = 'sha384-Jbsp01bfi5QMUu9TQeO+5kXvBTqk5CQkcSgbSP9rSEDuQnaamwBF7YDKrSsBmfXw';`,
