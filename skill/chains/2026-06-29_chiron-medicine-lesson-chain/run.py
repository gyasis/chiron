...
async def json_with_repair(prompt: str, name: str, model_dict: dict, validate_fn=None, max_repair: int = 3):
    """SELF-REPAIR LOOP (pharos REPAIR pattern) + MODEL FALLBACK: call the model; if the JSON won't parse OR
    fails validate_fn, re-prompt it with the previous output + the exact problem and loop (≤max_repair). If it
    still can't, fall back to the next model in the ladder (primary + CH_MODEL_FALLBACKS). On exhaustion of
    every model returns None (caller flags needs_review and KEEPS GOING — a bad chapter must not kill the
    whole lesson). This is LOOP #1/#3 from the investigation, made real."""
