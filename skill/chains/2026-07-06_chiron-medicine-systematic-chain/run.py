...
async def phase1(source: str):
    ...
    filled = await llm_json(model_for(MODEL_REASON), plan_p, "syllabus")
...
async def main():
    ...
    if syl_path.exists() and os.environ.get("CH_FORCE") != "1":
        ...
        obs.phase("Planning the sections", "start")
        syl = await phase1(source)
        obs.phase("Planning the sections", "end")
    ...
    # chapters + assemble, else there is no lesson.html and the bake fails "lesson.html not found".
    ...
    clips = 0
    # 'scripts' = transcript-ONLY backfill: write audio-scripts.json from the EXISTING chapters and stop.
    # It deliberately skips chapter authoring AND assemble (above), so lesson.html + chapters are NEVER
    # touched — only the missing narration manifest is added. Used to backfill lessons made at stage=assemble.
    ...
    if STAGE in ("scripts", "assemble", "audio", "all"):
