...
const OUT_DIR = resolve(process.env.HOME, 'Documents', 'generated', 'chiron-blood-chemistry-amboss');
...
// ── lesson metadata ───────────────────────────────────────────────────────────
const lessonTitle = brief.metadata?.topic ?? brief.topic ?? 'Blood Chemistry — Derangements & Their Acute Presentations';
const lessonDate = new Date().toISOString().slice(0, 10);
const lessonId = 'chiron-blood-chemistry-amboss';
