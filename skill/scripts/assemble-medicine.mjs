 ...
 *  - Lesson dir: ~/Documents/generated/chiron-blood-chemistry-amboss/
...
// Generalized from build-bloodchem-assemble.mjs (D4c — one data-driven assembler per chain).
// Usage: node assemble-medicine.mjs <lesson-output-dir>   (reads chapter*.json, emits lesson.html)
const OUT_DIR = process.argv[2]
  ? resolve(process.argv[2])
  : resolve(process.env.HOME, 'Documents', 'generated', 'chiron-blood-chemistry-amboss');
...
function renderWidgetSafe(spec, chapterIdx) {
  try {
    // Deterministic field-alias normalization (shared with render-check.mjs) — maps the
    // reasonable-but-wrong field names the LLM emits onto the exact shapes the renderers want.
    spec = normaliseWidget(spec);
...
const chapters = [];
const chapterNums = readdirSync(OUT_DIR)
  .map(f => (f.match(/^chapter(\d+)\.json$/) || [])[1])
  .filter(Boolean).map(Number).sort((a, b) => a - b);
if (chapterNums.length === 0) { process.stderr.write(`[assemble] no chapter*.json in ${OUT_DIR}\n`); process.exit(1); }
for (const i of chapterNums) {
...
// ── end-of-lesson GLOSSARY — aggregate every glossary-tooltips entry across chapters,
//    dedup by term, alphabetize. A consolidated reference section (mirrors the SR deck). ──
const _esc = s => String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
const _glossary = new Map();
const _add = (term, def) => {
  term = (term ?? '').trim();
  if (term && !_glossary.has(term.toLowerCase())) _glossary.set(term.toLowerCase(), { term, def: (def ?? '').trim() });
};
// PREFER a dedicated glossary.json (a real curated glossary); else aggregate glossary-tooltips.
const _glossaryJson = resolve(OUT_DIR, 'glossary.json');
if (existsSync(_glossaryJson)) {
  try {
    const g = JSON.parse(readFileSync(_glossaryJson, 'utf8'));
    for (const e of (Array.isArray(g) ? g : g.entries || g.terms || []))
      _add(e.term || e.word || e.name, e.definition || e.def || e.meaning || e.text);
  } catch { /* fall through to aggregation */ }
}
if (_glossary.size === 0) {
  for (const ch of chapters)
    for (const w of (ch.widgets || []))
      if (w.type === 'glossary-tooltips')
        for (const e of (w.entries || w.terms || w.items || w.glossary || []))
          _add(e.term || e.word || e.name || e.label, e.definition || e.def || e.meaning || e.description || e.text);
}
const glossaryEntries = [..._glossary.values()].sort((a, b) => a.term.localeCompare(b.term));
const glossaryHtml = glossaryEntries.length === 0 ? '' : `
    <section class="glossary-section" id="glossary">
      <h2>Glossary</h2>
      <p class="glossary-help">${glossaryEntries.length} key terms from this lesson, alphabetized.</p>
      <dl class="glossary-list">
        ${glossaryEntries.map(e => `<dt>${_esc(e.term)}</dt><dd>${_esc(e.def)}</dd>`).join('\n        ')}
      </dl>
    </section>`;
process.stderr.write(`[assemble] glossary: ${glossaryEntries.length} terms\n`);
...
<a class="toc-link" href="#glossary" data-chapter-target="glossary">
    <span class="toc-num">·</span><span class="toc-title">Glossary</span>
  </a>` : '');

// ── lesson metadata ───────────────────────────────────────────────────────────
const lessonTitle = brief.metadata?.topic ?? brief.topic ?? brief.metadata?.subject ?? brief.subject
  ?? OUT_DIR.split('/').pop().replace(/^chiron-|-amboss$/g, '').replace(/-/g, ' ')
       .replace(/\b\w/g, c => c.toUpperCase());
const lessonDate = new Date().toISOString().slice(0, 10);
const lessonId = OUT_DIR.split('/').pop();
...
${srCardCss}
  ...
  <!-- Audio manifest loader: sets window.__chironAudioManifest BEFORE the player init, so audio plays on
       file:// (browsers BLOCK fetch('audio/manifest.json') on file://). Written by the bake; 404s silently
       until then. THIS is what makes the 🎧 Listen player appear when a lesson is opened directly. -->
...
${glossaryHtml}
    ...
    <section class="sr-deck-section" id="sr-deck">
      ...
      <p class="sr-deck-help">Click any card to flip. Front = concept. Back = key fact + source tags. All ${allSrIdioms.length} cards from all 6 chapters.</p>
...
const totalWidgets = totalRendered + totalFailed;   // actual render targets (top-level specs; variants render client-side)
const pass = totalFailed === 0 ? '✅ ALL WIDGETS RENDER' : `❌ ${totalFailed} FAILED`;
process.stderr.write(`[assemble] Total rendered: ${totalRendered} / ${totalWidgets} widgets (${totalFailed} failed) — ${pass}\n`);
