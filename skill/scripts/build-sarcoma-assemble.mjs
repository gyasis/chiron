#!/usr/bin/env node
/**
 * Stage 5 Assemble — Sarcoma of the Chest AMBOSS lesson.
 *
 * Drives the REAL compiled widget-renderer dispatch (renderWidget from
 * ../dist/lib/widget-renderer.js) for all 38 widgets across 5 chapters,
 * emits the 6-theme-linked lesson.html, and inits .chiron-state.db.
 *
 * Theme contract:
 *   - 6 <link> tags pointing to themes/ dir (already copied by prior stages)
 *   - <html data-theme="clinical">
 *   - ?theme= + localStorage switcher script
 *   - NO hardcoded hex in component CSS — only var(--chiron-*) tokens
 */
...
const OUT_DIR = resolve(process.env.HOME, 'Documents', 'generated', 'chiron-sarcoma-of-the-chest-amboss');
...
// Each chapter SR card: {front, back, tags} → map to idiom shape
// {it: front, meaning: back, literal: tags?.join(', ')}
...
/**
 * Convert inline markdown to HTML in a string that has already been HTML-escaped
 * by the widget renderer (so we work on the escaped output, not raw source).
 * Patterns: **text** → <strong>text</strong>, *text* → <em>text</em>, `text` → <code>text</code>
 * NOTE: the widget renderer calls escapeHtml() so literal asterisks/backticks
 * are NOT escaped (they are not HTML special chars); this runs AFTER rendering.
 */
...
function renderChapter(ch, idx) {
  ...
  const prose = ch.exposition ?? ch.narrativeHtml ?? '';

  const widgetHtmlParts = (ch.widgets ?? []).map(spec => {
    const rawHtml = renderWidgetSafe(spec, chNum);
    // FIX 3: convert leftover **bold**, *em*, `code` markdown in widget output
...
// ── lesson metadata ───────────────────────────────────────────────────────────
const lessonTitle = brief.topic ?? 'Sarcoma of the Chest';
const lessonDate = new Date().toISOString().slice(0, 10);
const lessonId = 'chiron-sarcoma-of-the-chest-amboss';
...
(function(){
  ...
  // URL ?theme= takes priority, then localStorage
...
// Inline the relevant bits from main.js for chat + flow + layer-toggle
const animationJs = `
<script>
...
${chaptersHtml}
    ...
    <section class="sr-deck-section" id="sr-deck">
      ...
      <p class="sr-deck-help">Click any card to flip. Front = concept. Back = key fact + source tags. All ${allSrIdioms.length} cards from all 5 chapters.</p>
...
// Inline the schema rather than depending on initDb (avoid import issues with ESM)
...
// Seed SR cards from all chapters
...
process.stderr.write(`[assemble] Total rendered: ${totalRendered} / 38 widgets (${totalFailed} failed)\n`);
